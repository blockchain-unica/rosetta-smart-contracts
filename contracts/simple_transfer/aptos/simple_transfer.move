module smart_contracts_comparison::simple_transfer {
    ...
    public fun deposit(sender: &signer, deposit_amount: Coin<AptosCoin>) acquires SimpleTransfer {
        let simple_transfer = borrow_global_mut<SimpleTransfer<AptosCoin>>(signer::address_of(sender));
    ...
    public fun withdraw(sender: &signer, owner: address, amount: u64) acquires SimpleTransfer {
        let simple_transfer = borrow_global_mut<SimpleTransfer<AptosCoin>>(owner);
